...
function formatDateForDisplay(value) {
  ...
  const vn = String(value).trim().match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})$/);
  if (vn) {
    const [, d, m, y] = vn;
    return `${d.padStart(2, "0")}/${m.padStart(2, "0")}/${y}`;
  }
...
async function refreshDependent(parentField, parentValue) {
  ...
  const payload = await getJson(
    `/api/filters/dependent?parent_field=${encodeURIComponent(parentField)}&parent_value=${encodeURIComponent(parentValue)}&form_state_id=${encodeURIComponent(appState.formStateId)}`,
  );
...
async function executeSearch(pageIndex = 1) {
  ...
  const body =
    pageIndex === 1 || !appState.previewId
...
function bindSearch() {
  ...
  form.querySelector("select[name='court_level']")?.addEventListener("change", (event) =>
    refreshDependent("court_level", event.target.value),
  );
  form.querySelector("select[name='case_style']")?.addEventListener("change", (event) =>
    refreshDependent("case_style", event.target.value),
  );
  ...
  document.getElementById("prev-page")?.addEventListener("click", async () =>
    executeSearch(Math.max(1, appState.currentPage - 1)),
  );
  document.getElementById("next-page")?.addEventListener("click", async () =>
    executeSearch(Math.min(appState.totalPages, appState.currentPage + 1)),
  );
    ...
    <article class="result-card">
      <header>
        <div>
          ...
          <div class="muted">Tạo lúc: ${job.created_at_display || job.created_at || ""}</div>
        </div>
...
function startJobsPolling() {
  if (!document.getElementById("jobs-list") || jobsPollTimer) return;
  loadJobs().catch(console.error);
  jobsPollTimer = window.setInterval(() => {
    if (document.hidden || jobsPollInFlight) return;
    loadJobs().catch(console.error);
  }, 2000);
}
    ...
    container.innerHTML = items
      ...
      <article class="result-card" data-search="${[document.document_number, document.court_name, document.document_type].join(" ").toLowerCase()}">
        ...
        <div class="meta-grid">
          ...
          <div><strong>Ngày ban hành:</strong> ${formatDateForDisplay(document.issued_date_display || document.issued_date || "")}</div>
  ...
  document.getElementById("document-filter")?.addEventListener("input", (event) => {
    ...
    render(
      documents.filter((item) =>
        [item.document_number, item.court_name, item.document_type].join(" ").toLowerCase().includes(query),
      ),
    );
  });
}
