...
class DetailJudgmentParser:
    def parse(self, html: str, source_url: str) -> dict[str, str]:
        ...
        return {
            ...
            "title": first_present([self._field(soup, "ten_ban_an"), extract_regex(r"Tên bản án:\s*([^\n]+)", text)]),
            "document_number": first_present([self._field(soup, "so_ban_an"), extract_regex(r"Số bản án:\s*([^\n]+)", text)]),
